    ...
    def write_results(self, show_missing=True, summary=False, coverdir=None):
        ...
        if self.calledfuncs:
            ...
            for filename, modulename, funcname in sorted(calls):
                print(("filename: %s/%d, modulename: %s, funcname: %s"
                       % (filename, lineno, modulename, funcname)))

        if self.callers:
            print()
            print("calling relationships:")
            lastfile = lastcfile = ""
            for ((pfile, pline, pmod, pfunc), (cfile, cline, cmod, cfunc)) \
                ...
                if cfile != pfile and lastcfile != cfile:
                    print("  -->", cfile)
                    lastcfile = cfile
                print("    %s.%s/%d -> %s.%s/%d" % (pmod, pfunc, pline,
                                                    cmod, cfunc, cline))
